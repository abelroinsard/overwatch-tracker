...
export default function PlayerInfo({ playerInfo }: PlayerInfoProps) {
  const getCareerStats = (platform: any, mode: "quickplay" | "competitive") => {
    if (!platform || !platform[mode] || !platform[mode].career_stats || !platform[mode].career_stats["all-heroes"]) {
      return 0;
    }
    ...
    const gameCategory = statsArr.find((category: any) => category.category === "game");
    if (!gameCategory || !gameCategory.stats) {
      return 0;
    }
    
    // Trouver le stat "time_played"
    const timePlayedStat = gameCategory.stats.find((stat: any) => stat.key === "time_played");
  ...
  const CompConsoleTime = getCareerStats(playerInfo.stats.console, "competitive");
  ...
  return (
    <div className="PlayerInfo">
      ...
      />
      <div className="bottomInfo"><PlayTimes QPTime={QPPCTime + QPConsoleTime} CompTime={CompPCTime + CompConsoleTime}></PlayTimes></div>
    </div>
  );
}
